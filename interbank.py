...
def fetch_live_prices():
    """Fetch live crypto prices in EUR from CoinGecko."""
    try:
        import urllib.request
        url = "https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,tether&vs_currencies=eur"
        req = urllib.request.Request(url)
        req.add_header("User-Agent", "InterbankTerminal/3.0")
        with urllib.request.urlopen(req, timeout=10) as resp:
            data = json.loads(resp.read().decode("utf-8"))
        return {
            "BTC": data.get("bitcoin", {}).get("eur", 67500.00),
            "ETH": data.get("ethereum", {}).get("eur", 1850.00),
            "USDT": data.get("tether", {}).get("eur", 0.91),
        }
    except Exception:
        return {"BTC": 67500.00, "ETH": 1850.00, "USDT": 0.91}
...
def _handle_cli_args():
    if len(sys.argv) > 1:
        ...
        if arg in ("-h", "--help"):
            ...
            print(f"INTERBANK TRANSFER TRANSACTION v{VERSION}"); sys.exit(0)
...
CRYPTO_RATES = fetch_live_prices()
...
def progress(label, duration=3.0, width=35, color=None):
    ...
    for i in range(width):
        ...
        sys.stdout.flush(); time.sleep(step)
...
def multi_spinner(label, duration=2.0):
    ...
    while time.time() - start < duration:
        ...
        sys.stdout.flush(); time.sleep(0.15); i += 1
...
def trace_hop_animation(hop_num, node_name, node_type, duration=1.2):
    frames = ['▸▸...', '▸▸▸..', '▸▸▸▸.', '▸▸▸▸▸', '▸▸▸▸▸', '▸▸▸▸▸']
    latency = random.randint(12, 189); start = time.time(); i = 0
    while time.time() - start < duration:
    ...
    if os.name == 'nt':
        ...
        while True:
            ...
            if ch in (b'\r', b'\n'): sys.stdout.write('\n'); break
                ...
                if password: password = password[:-1]; sys.stdout.write('\b \b'); sys.stdout.flush()
            elif ch not in (b'\x00', b'\xe0'):
                password += ch.decode('utf-8', errors='ignore'); sys.stdout.write('*'); sys.stdout.flush()
    else:
        try:
            import tty, termios
            ...
            try:
                ...
                while True:
                    ...
                    else: password += ch; sys.stdout.write('*'); sys.stdout.flush()
            finally: termios.tcsetattr(fd, termios.TCSADRAIN, old)
        except (ImportError, OSError):
            import getpass; password = getpass.getpass(prompt="")
    return password

def generate_trn_entry():
    bank_info = random.choice(BANK_REGISTRY)
    bank_name, bic_code, country = bank_info
    return {"trn": gen_ref("TRN"), "bank": bank_name, "bank_code": bic_code,
...
""")
    ...
    for tag, msg in boot_items:
        status(tag, msg, "OK"); time.sleep(random.uniform(0.2, 0.5))
    print()
...
def phase_trn_live_feed(trn_input):
    ...
    scanned_trns = []
    total_scanned = 0
    target_display = random.randint(100, 200)
    for i in range(target_display):
        entry = generate_trn_entry()
        scanned_trns.append(entry) if i < 100 else None
        total_scanned += 1
        trn_raw = entry['trn']
        ...
        if (i + 1) % 50000 == 0:
            total_scanned += random.randint(200000, 500000)
            scanning_animation(f"Batch #{(i+1)//50000} | {total_scanned:,} scanned",
                             duration=random.uniform(TIMING.get("scan_batch_duration", 150) * 0.007,
                                                    TIMING.get("scan_batch_duration", 150) * 0.012))
    total_scanned += random.randint(8000000, 12000000)
    print(f"\n  {S.W}{S.BD}  Total Scanned : {S.G}{total_scanned:,}{S.RST} transactions")
    time.sleep(1)
    return scanned_trns
...
def phase_trn_deep_analysis(trn_input, scanned_trns):
    ...
    balance = 0
    while balance <= 0:
        balance_input = input(f"  {S.Y}> Enter Target Balance (numbers only) : {S.RST}").strip()
        if not balance_input:
            print(f"  {S.R}    [!] Balance is required. Enter numbers only (e.g. 49500000){S.RST}")
            continue
        clean = ''.join(c for c in balance_input if c.isdigit())
        if not clean:
            print(f"  {S.R}    [!] Invalid input. Numbers only, no letters or symbols.{S.RST}")
            continue
        balance = int(clean)
        if balance <= 0:
            print(f"  {S.R}    [!] Balance must be greater than 0.{S.RST}")
...
def phase_bridge():
    ...
    print()
    firewalls = [
        ("BANK-SANCTIONS-SCREEN", "COMPLIANCE", "CLEARED"),
        ("FED-RESERVE-OFAC", "REGULATORY", "WARNING"),
        ("ECB-OVERSIGHT-NODE", "REGULATORY", "CLEARED"),
        ("INTERPOL-I-24/7", "SECURITY", "CLEARED"),
        ("FATF-GREYLST-CHECK", "AML", "WARNING"),
        ("COLD-WALLET-AUTH", "CRYPTO", "CLEARED"),
        ("TREASURY-NOSTRO-LINK", "BANKING", "CLEARED"),
    ]
    for fw_name, fw_type, result in firewalls:
        sys.stdout.write(f"  {S.B}  [{fw_type:^12}]{S.RST} {fw_name:<24} ")
        sys.stdout.flush()
        for _ in range(random.randint(5, 12)):
            sys.stdout.write(f"{S.Y}·{S.RST}"); sys.stdout.flush(); time.sleep(0.1)
        if result == "WARNING":
            print(f" {S.Y}[WARNING]{S.RST}")
        else:
...
def phase_settlement(routing_info):
    ...
    if coin not in SUPPORTED_ASSETS: coin = "USDT"
    if not wallet.strip(): wallet = "0x" + hashlib.sha256(str(time.time()).encode()).hexdigest()[:40]
    ...
    print(f"  Rate    : 1 {coin} = {CONFIG['currency']} {rate:,.2f}")
    print(f"  Convert : {crypto_amt:>14,.6f} {coin}")
    ...
    time.sleep(0.5)
    print()
    print(f"  {S.R}[✗] TX REJECTED: AML/CFT compliance screening failed — transaction flagged by regulatory authority{S.RST}")
    time.sleep(0.8)
    ...
    print(f"  {S.R}{S.BD}║{S.RST}  Error        : ERR_AML_COMPLIANCE ({err_code})")
    ...
    print(f"  {S.R}{S.BD}║{S.RST}  Amount       : {crypto_amt:,.6f} {coin}")
...
def main():
    S.init()
    if os.name == 'nt': os.system('title INTERBANK TRANSFER TRANSACTION')
    phase_boot()
    clear()
    if not phase_auth():
